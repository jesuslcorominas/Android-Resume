    ...
    final void enqueueCall(Call<ServerResponse<List<T>>> call, final RestClient.ListCallback<T> callback) {
        call.enqueue(new Callback<ServerResponse<List<T>>>() {
            ...
            public void onFailure(Call<ServerResponse<List<T>>> call, Throwable t) {
                callback.onError(new ErrorInfo(Keys.ResultCodes.serverError, "Error al pedir el listado de datos personales: " + t.getMessage()));
            }
        });
    }

    final void enqueueCall(Call<ServerResponse<T>> call, final RestClient.DetailCallback<T> callback) {
        call.enqueue(new Callback<ServerResponse<T>>() {
            @Override
            ...
            public void onFailure(Call<ServerResponse<T>> call, Throwable t) {
                callback.onError(new ErrorInfo(Keys.ResultCodes.serverError, "Error al hacer la peticion de detalle de datos personales: " + t.getMessage()));
            }
        });
    }
}
